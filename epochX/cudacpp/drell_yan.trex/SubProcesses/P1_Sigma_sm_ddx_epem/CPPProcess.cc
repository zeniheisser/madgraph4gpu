...
#endif
{
  ...
  constexpr int nIPC = 6; // SM independent couplings used in this CPPProcess.cc (FIXME? rename as sm_IndepCoupl?)
  ...
  __device__ const fptype cIPC[nIPC * 2] = { (fptype)Parameters_sm::GC_2.real(), (fptype)Parameters_sm::GC_2.imag(), (fptype)Parameters_sm::GC_3.real(), (fptype)Parameters_sm::GC_3.imag(), (fptype)Parameters_sm::GC_50.real(), (fptype)Parameters_sm::GC_50.imag(), (fptype)Parameters_sm::GC_58.real(), (fptype)Parameters_sm::GC_58.imag(), (fptype)Parameters_sm::GC_59.real(), (fptype)Parameters_sm::GC_59.imag(), (fptype)Parameters_sm::GC_1.real(), (fptype)Parameters_sm::GC_1.imag() };
#else
#ifdef MGONGPUCPP_GPUIMPL
...
#pragma nv_diagnostic pop
      ...
      FFV1P0_3<W_ACCESS, CI_ACCESS>( w_fp[0], w_fp[1], COUPs[ndcoup + 5], 1.0, 0., 0., w_fp[4] );

      // Amplitude(s) for diagram number 1
      FFV1_0<W_ACCESS, A_ACCESS, CI_ACCESS>( w_fp[2], w_fp[3], w_fp[4], COUPs[ndcoup + 1], 1.0, &amp_fp[0] );
#ifdef MGONGPU_SUPPORTS_MULTICHANNEL
      ...
      FFV2_3_3<W_ACCESS, CI_ACCESS>( w_fp[0], w_fp[1], COUPs[ndcoup + 2], 1.0, COUPs[ndcoup + 3], 1.0, cIPD[0], cIPD[1], w_fp[4] );

      // Amplitude(s) for diagram number 2
      FFV2_4_0<W_ACCESS, A_ACCESS, CI_ACCESS>( w_fp[2], w_fp[3], w_fp[4], COUPs[ndcoup + 2], 1.0, COUPs[ndcoup + 4], 1.0, &amp_fp[0] );
#ifdef MGONGPU_SUPPORTS_MULTICHANNEL
      // Here the code base generated with multichannel support updates numerators_sv and denominators_sv (#473)
#endif
      jamp_sv[0] += amp_sv[0];
      ...
      // *** COLOR MATRIX BELOW ***
      // (This method used to be called CPPProcess::matrix_1_ddx_epem()?)
      ...
      // [NB do keep 'static' for these constexpr arrays, see issue #283]
      static constexpr fptype2 cf[ncolor][ncolor] = { { 3 } }; // 2-D array[1][1]
  ...
  CPPProcess::initProc( const std::string& param_card_name )
  {
    ...
    const cxtype tIPC[nIPC] = { cxmake( m_pars->GC_2 ), cxmake( m_pars->GC_3 ), cxmake( m_pars->GC_50 ), cxmake( m_pars->GC_58 ), cxmake( m_pars->GC_59 ), cxmake( m_pars->GC_1 ) };
#ifdef MGONGPUCPP_GPUIMPL
